define(['jquery','underscore','Magento_Checkout/js/view/shipping','Magento_Checkout/js/model/quote','Magento_Customer/js/model/customer','Magento_Checkout/js/action/set-shipping-information','Mageplaza_Osc/js/action/payment-total-information','Magento_Checkout/js/model/step-navigator','Magento_Checkout/js/model/payment/additional-validators','Magento_Checkout/js/checkout-data','Magento_Checkout/js/action/select-billing-address','Magento_Checkout/js/action/select-shipping-address','Magento_Checkout/js/model/address-converter','Mageplaza_Osc/js/model/shipping-rate-service','Magento_Checkout/js/model/shipping-service','Mageplaza_Osc/js/model/checkout-data-resolver','Mageplaza_Osc/js/model/address/auto-complete','Mageplaza_Osc/js/model/compatible/amazon-pay','Magento_Customer/js/model/address-list','rjsResolver','uiRegistry','mage/translate'],function($,_,Component,quote,customer,setShippingInformationAction,getPaymentTotalInformation,stepNavigator,additionalValidators,checkoutData,selectBillingAddress,selectShippingAddress,addressConverter,shippingRateService,shippingService,oscDataResolver,addressAutoComplete,amazonPay,addressList,resolver,registry){'use strict';oscDataResolver.resolveDefaultShippingMethod();shippingService.setShippingRates(window.checkoutConfig.shippingMethods);return Component.extend({defaults:{template:'Mageplaza_Osc/container/shipping',shippingMethodItemTemplate:'Mageplaza_Osc/shipping-address/shipping-method-item'},currentMethod:null,isAmazonAccountLoggedIn:amazonPay.isAmazonAccountLoggedIn,initialize:function(){this._super();if(window.checkoutConfig.hasOwnProperty('amazonLogin')){this.isNewAddressAdded(this.isAmazonAccountLoggedIn());this.isAmazonAccountLoggedIn.subscribe(function(value){this.isNewAddressAdded(value);},this);}
...
var billingScope=registry.get("checkout.steps.shipping-step.billingAddress");this.isSelected.subscribe(function(value){if(value==='instore_pickup'){if(billingScope.isAddressSameAsShipping()){$('#billing-address-same-as-shipping').trigger('click');};$('#shipping').hide();$('#store-pickup').show();}else{$('#shipping').show();$('#store-pickup').hide();}});if(this.isSelected()==='instore_pickup'){billingScope.isAddressSameAsShipping(false);$('#shipping').hide();$('#store-pickup').show();}else{$('#shipping').show();$('#store-pickup').hide();}},validate:function(){if(this.isAmazonAccountLoggedIn()){return true;}
if(quote.isVirtual()){return true;}
if(quote.shippingMethod()&&quote.shippingMethod().carrier_code==='instore'){return true;}
...
selectShippingAddress(shippingAddress);},saveNewAddress:function(){this._super();if(!this.source.get('params.invalid')){shippingRateService.isAddressChange=true;shippingRateService.estimateShippingMethod();}},getPopUp:function(){if(this.popUpForm.element==='#opc-new-shipping-address'){this.popUpForm.options.modalClass='osc-new-address-popup'}
return this._super();},getAddressTemplate:function(){return'Mageplaza_Osc/container/address/shipping-address';}});});