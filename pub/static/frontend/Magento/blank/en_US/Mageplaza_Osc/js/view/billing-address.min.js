...
if(canShowBillingAddress){oscData.setData('same_as_shipping',same_as_shipping);}
...
return true;},onAddressChange:function(address){this.isAddressFormVisible(address===newAddressOption);if(!this.isAddressSameAsShipping()&&canShowBillingAddress){this.updateAddress();}},updateAddress:function(){var self=this,newBillingAddress,selectedAddress,addressData;if(this.selectedAddress()&&!this.isAddressFormVisible()){newBillingAddress=createBillingAddress(this.selectedAddress());selectedAddress={customerAddressId:this.selectedAddress().customerAddressId,customerId:this.selectedAddress().customerId,sameAsBilling:this.selectedAddress().sameAsBilling,regionId:this.selectedAddress().regionId,getAddressInline:function(){return self.selectedAddress().getAddressInline();}};selectBillingAddress($.extend(newBillingAddress,selectedAddress));checkoutData.setSelectedBillingAddress(this.selectedAddress().getKey());}else{addressData=this.source.get('billingAddress');if(customer.isLoggedIn()&&!this.customerHasAddresses){this.saveInAddressBook(1);addressData.save_in_address_book=1;}else{addressData.save_in_address_book=0;}
if(addressData.custom_attributes){_.each(addressData.custom_attributes,function(value,key){if(_.isEmpty(value)){delete addressData.custom_attributes[key];}});}
...
oscData.setData('same_as_shipping',false);return!this.source.get('params.invalid');},getAddressTemplate:function(){return'Mageplaza_Osc/container/address/billing-address';}});});