...
class Api
{
     ...
     *
     * @return Ambigous <\Laminas\Http\Response, string, \Laminas\Http\Response\Stream, unknown, \Laminas\Http\Response\Stream>
     */
    public function getKeyInfo($licenseKey)
    {
        ...
        try {
            $response = $this->serializer->unserialize($this->decode($response->getBody()));
        } catch (\Exception $e) {
    ...
    public function getExtensionsList()
    {
        ...
        if (!$fileExist || $reupdate) {
            ...
            try {
                $extensionsArr = $this->serializer->unserialize($this->decode($response));
        ...
        try {
            ...
            $extensionLists = $this->serializer->unserialize($this->decode($extensionLists));
    ...
    public function remoteSaveLicenseKey($licenseKey, $domains = [])
    {
        ...
        $domains = $this->serializer->serialize($domains);
        $client->setParameterPost([
    ...
    public function getSavedKeyInfo(\Vnecoms\Core\Model\Key $key)
    {
        ...
        try {
            $result = $this->serializer->unserialize($this->decode($key->getLicenseInfo(), self::ENCODED_KEY));
        } catch (\Exception $e) {
            $result = false;
        }
