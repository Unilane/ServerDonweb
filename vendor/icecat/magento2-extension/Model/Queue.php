...
class Queue
{
    ...
    {
		$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/icecatSP2.log');
                $logger = new \Zend_Log();
                $logger->addWriter($writer);
		$logger->info("inicio del diseño de las especificaciones");
		$logger->info("");
        $authResponse = $this->data->validateToken();
        if ($authResponse['httpcode'] != "400") 
        {
            ...
            foreach ($jobs as $job) {
			    ...
			    $logger->info("sku del procucto"." ".$product->getSku());
                            if ($icecatUri) {                                
				    ...
				    $logger->info("sku siguiente"." ".$product->getSku());
                		    ...
                		    $logger->info("sku actualizado"." ".$product->getSku());
                                }
                            } else {                                
