...
class MassDelete extends Action
{
        ...
        if (!empty($productsIds)) {
            try {
                ...
                $taskData = [
                    ...
                    'action' => ProductTaskQueue::ACTION_DELETE
                ];
                ...
                foreach ($productsIds as $productId) {
                    ...
                    if (!empty($productMadktingId = $madktingProduct->getMadktingProductId())) {
                        try {
                            ...
                            $madktingProduct->setStatus(Product::STATUS_DELETING)
                ...
                if ($count > 0) {
                    $response['message'] = __('Deleting %1 %2 in Yuju', $count, $count > 1 ? __('products') : __('product'));
                } else {
                    $response['message'] = __('There are no new products to delete in selection');
                }
            } catch (\Exception $e) {
                $response['message'] = __('Something went wrong deleting products in Yuju');
    ...
    {
        return $this->_authorization->isAllowed('Madkting_Connect::product_delete');
    }
}
